...
#### 2.1.1 规则
- 点击起始节点（Start）可以直接点亮
- 点亮一个节点后，其上下左右相邻的节点变为"可达"状态（绿色边框）
- 只有可达节点可以被点亮
- 已点亮的节点保持高亮显示（红色边框 + 光晕效果）
- Gate节点特殊处理：入口Gate可点亮，出口Gate弹出选择框
...
#### 2.2.1 Gate节点分类
Gate节点是位于盘面边缘的特殊节点，分为两种状态：

| 状态 | 说明 | 行为 |
|------|------|------|
| **入口Gate** | 被其他盘面连接的Gate | 可被点亮，成为进入该盘面的入口 |
| **出口Gate** | 未被连接的Gate | 点击后弹出面板选择框，连接新盘面 |
...
5. 新盘面的对应Gate变为入口（可点亮）

#### 2.2.4 盘面替换
- 已连接的Gate再次点击时，支持替换为其他盘面
- 替换时保留位置和旋转属性，只更换盘面内容

### 2.3 边挨边坐标系统

#### 2.3.1 问题背景
初始实现中，使用了原始项目的坐标系统（相邻位置间距3350像素），但由于节点尺寸和间距不同（原始项目：CELL_SIZE=150，本实现：CELL_SIZE=40），导致盘面之间出现巨大间隙。
...
#### 2.6.1 功能概述
快捷路径模式允许用户一键点亮从已点亮区域到目标节点的最短路径，无需逐一点击每个节点。
...
const findShortestPath = (
  ...
  // 使用BFS遍历已点亮节点，找到所有连通节点
  
  // 2. 使用连通的已点亮节点作为起点进行BFS
  // 找到到目标节点的最短路径
...
#### 2.6.4 跨盘路径计算
算法支持跨多个盘面计算路径：
- 遇到入口Gate节点时，自动跳转到子盘的入口Gate
- 继续在子盘中搜索路径

#### 2.6.5 Gate节点特殊处理
- **非入口Gate**：点击时弹出选择框，不计算路径
- **入口Gate**：
  - 第一次点击（快捷路径模式）：同时计算并点亮到该Gate的路径 + 弹出选择框连接新盘面
  - 第二次点击（已连接）：弹出选择框进行盘面替换

#### 2.6.6 最大盘面限制
- 最多支持5个盘面（1个起始盘 + 4个连接盘）
- 达到限制时，点击Gate节点不弹出选择框
